...
function sortArrayDate(projectID) {
  ...
  let unchecked = [];
  tasks.forEach((task) => {
    if (task.task_checked == false) unchecked.push(task);
  });
  ...
  let checked = [];
  tasks.forEach((task) => {
    if (task.task_checked == true) checked.push(task);
  });
...
function newProjectBtn() {
  ...
  document.addEventListener("click", (e) => {
    if (
      !newProjectForm.contains(e.target) &&
...
function setToday() {
  let today = new Date(new Date().toLocaleString().slice(0, 9))
    .toISOString()
    .slice(0, 10);
